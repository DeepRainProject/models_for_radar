...
if my_rank == p-1:
    ...
    model.add(tf.keras.layers.Conv2DTranspose(32, kernel_size=(7,7), strides=2, padding='same', activation='relu'))
    model.add(tf.keras.layers.BatchNormalization())
    model.add(tf.keras.layers.Conv2D(1, kernel_size=(3,3), strides=1, activation='relu', padding='same'))
                        
    model.compile(optimizer, loss=loss)
    
    if my_rank == 0:
        print('Trn dataset =', trn_dataset)
        print('Vld dataset =', vld_dataset)
        model.summary()
